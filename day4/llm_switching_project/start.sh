...
if [ ! -f "metrics.json" ]; then
    echo '{"total_requests": 0, "openai_requests": 0, "anthropic_requests": 0, "total_latency_ms": 0, "openai_latency_ms": 0, "anthropic_latency_ms": 0, "errors": 0, "last_request_time": null, "requests": []}' > metrics.json
fi

# Start dashboard in background
echo "Starting LLM Switcher Dashboard..."
...
nohup python background_demo.py --interval 5 > background_demo.log 2>&1 &
DEMO_PID=$!
echo "Background demo started with PID: $DEMO_PID"
echo "New requests will be added every 5 seconds"
