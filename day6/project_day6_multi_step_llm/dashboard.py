...
<html>
<head>
    <title>Multi-Step LLM Article Processor Dashboard</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>🚀 Multi-Step LLM Article Processor Dashboard</h1>
                ...
                .then(data => {
                    ...
                    document.getElementById('metricsGrid').innerHTML = `
                        ...
                        <div class="metric-card">
                            ...
                            <div class="metric-value">${formatNumber(metrics.successful_processing)}</div>
                            <div class="metric-label">Completed</div>
                        </div>
                        <div class="metric-card">
                            <h3>Failed Processing</h3>
                            <div class="metric-value">${formatNumber(metrics.failed_processing)}</div>
                            <div class="metric-label">Errors</div>
                        </div>
                        <div class="metric-card">
                            <h3>Step 1: Summaries</h3>
                            <div class="metric-value">${formatNumber(metrics.step1_summaries)}</div>
                            <div class="metric-label">Summaries Generated</div>
                        </div>
                        <div class="metric-card">
                            <h3>Step 2: Rewrites</h3>
                            <div class="metric-value">${formatNumber(metrics.step2_rewrites)}</div>
                            <div class="metric-label">Rewrites Generated</div>
                        </div>
                        <div class="metric-card">
                            <h3>Step 3: Keywords</h3>
                            <div class="metric-value">${formatNumber(metrics.step3_keywords)}</div>
                    ...
                    if (recentRequests.length === 0) {
                        ...
                        requestsBody.innerHTML = recentRequests.map(req => {
                            const steps = [];
                            if (req.step1) steps.push('Step 1');
                            if (req.step2) steps.push('Step 2');
                            if (req.step3) steps.push('Step 3');
                            const stepsText = steps.length > 0 ? steps.join(', ') : 'None';
                            return `
                                <tr>
...
if __name__ == '__main__':
    print("Starting Multi-Step LLM Article Processor Dashboard on http://localhost:5000")
