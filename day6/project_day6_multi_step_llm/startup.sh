...
if [ ! -f "metrics.json" ]; then
    echo '{"total_requests": 0, "successful_processing": 0, "failed_processing": 0, "step1_summaries": 0, "step2_rewrites": 0, "step3_keywords": 0, "total_latency_ms": 0, "average_latency_ms": 0.0, "last_request_time": null, "requests": []}' > metrics.json
fi

# Start dashboard in background
echo "Starting Multi-Step LLM Article Processor Dashboard..."
...
nohup python background_demo.py --interval 15 > background_demo.log 2>&1 &
DEMO_PID=$!
echo "Background demo started with PID: $DEMO_PID"
echo "New requests will be added every 15 seconds"
